...
is a `<ul>`, `<ol>`, or `<menu>` and a `<div>` anywhere else, so once
mounted it is valid HTML in either. The container isn’t knowable until the
item is in the DOM, so it mounts as an `<li>` and switches before paint
...
pass `as` when it must be valid too.
