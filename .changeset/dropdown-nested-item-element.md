...
container is a `<ul>`, `<ol>`, or `<menu>` and a `<div>` anywhere else. The
container isn’t knowable until the item is in the DOM, so it mounts as an
`<li>` and switches before paint when the container turns out not to be a
list; a nested Dropdown inside a list renders exactly as before.
